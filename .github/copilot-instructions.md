...
> **バージョン:** 1.1.0  
> **最終更新:** 2026 年 1 月 17 日  
> **作成モデル:** Claude Sonnet 4.5  
> **対象プロジェクト:** react-todo-state-management（全体共通ルール）
...
このリポジトリには3つの異なる状態管理パターンの実装が含まれています：

#### 01-custom-hooks

- **グローバル状態管理ライブラリは不使用**
- **カスタムフック（`useTodo.js`）で集約** - すべての Todo 関連の状態とロジック
- Props でデータを親から子へ渡す（Lift State Up パターン）

#### 02-context-api

- **Context API を使用** - `TodoContext.jsx` でアプリ全体の状態を管理
- **カスタムフック（`useTodoContext.js`）で Context へのアクセスを簡略化**
- Props Drilling を回避し、深い階層でも状態にアクセス可能

#### 03-redux

- **Redux を使用** - `store/` ディレクトリで状態管理
- Actions、Reducers、Store の構成（実装詳細は各プロジェクト内の指示を参照）

#### 共通のルール

- **イミュータブルな更新** - スプレッド構文で新しい配列・オブジェクトを作成
- 状態の変更は必ず適切な方法（useState / Context / Redux）を通じて行う
...
各プロジェクトで状態管理の方法が異なりますが、基本的なコンポーネント階層は共通です：

```
constants/data.js (初期データ: INITIAL_TODOS, INITIAL_UNIQUE_ID)
    ↓
【状態管理層】
  01: hooks/useTodo.js (カスタムフック)
  02: contexts/TodoContext.jsx (Context API)
  03: store/ (Redux)
    ↓
...
- [ ] プロジェクトに応じた適切な状態管理手法を使用しているか（Custom Hooks / Context API / Redux）
...
- ワークスペースルートに配置し、全体共通ルールに変更
- 状態管理のセクションを01-03の違いを明示した構成に修正
- データフローを各プロジェクトの状態管理手法に対応
